// Reiseziele ("Ziele") — portiert aus vanlifemum-boop/tussy-van-website (js/destinations.js).
// Texte unverändert übernommen. `slug` (= id) ist ein stabiler Anker, damit später
// Blog-Beiträge (Reisen mit Kind) je Ziel verknüpft werden können.
...
export type HeadColor = "bg-blue" | "bg-deep" | "bg-army" | "bg-terra" | "bg-camel";
...
export const COUNTRIES: { key: CountryKey; label: string; flag: string }[] = [
  { key: "polen", label: "Polen", flag: "🇵🇱" },
  { key: "deutschland", label: "Deutschland", flag: "🇩🇪" },
  { key: "daenemark", label: "Dänemark", flag: "🇩🇰" },
  { key: "niederlande", label: "Niederlande", flag: "🇳🇱" },
  { key: "frankreich", label: "Frankreich", flag: "🇫🇷" },
  { key: "spanien", label: "Spanien", flag: "🇪🇸" },
  { key: "italien", label: "Italien", flag: "🇮🇹" },
  { key: "kroatien", label: "Kroatien", flag: "🇭🇷" },
  { key: "oesterreich", label: "Österreich", flag: "🇦🇹" },
];

const raw: Omit<Destination, "slug">[] = [
  {
...
export const DESTINATIONS: Destination[] = raw.map((d) => ({ ...d, slug: d.id }));
